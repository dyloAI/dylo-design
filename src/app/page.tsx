  ...
  return (
    <Page>
      <PageHeader
        ...
        lead={
          <>
            The tokens, Tailwind theme, components and brand assets behind dylo. Install any piece straight from
            GitHub with the shadcn CLI — no package to publish, no registry server to run.
          </>
        }
      />
      ...
      >
        ...
        <p className="text-concrete mt-3 mb-0 text-[14px]">
      ...
      <Section title="The non-negotiables" description="Break any of these and the work stops looking like dylo.">
        <dl className="border-line grid gap-0 rounded-sm border sm:grid-cols-2">
          {NON_NEGOTIABLES.map(([term, detail]) => (
            <div key={term} className="border-line border-b p-5 last:border-b-0 sm:[&:nth-last-child(-n+2)]:border-b-0">
              <dt className="text-ink text-[15px] font-semibold">{term}</dt>
              <dd className="text-graphite m-0 mt-1.5 text-[14px]/[1.6]">{detail}</dd>
            </div>
          ))}
        </dl>
      </Section>
      ...
      <Section title="What is in here">
        <ul className="text-graphite m-0 list-none space-y-2 p-0 text-[15px]">
          {[
            ...
            <li key={href} className="border-line border-b pb-2 last:border-b-0">
              <Link href={href} className="text-ink font-medium">
                {title}
              </Link>
        ...
        <p className="text-concrete mt-6 mb-0 text-[14px]">
          Source lives at{' '}
